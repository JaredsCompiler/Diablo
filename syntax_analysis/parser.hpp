...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // term
      char dummy2[sizeof ( std::vector<double> )];

      // expression
      char dummy3[sizeof ( std::vector<long long int> )];
    ...
    TOKEN_ASSIGN = 264,            // "assignment"
    TOKEN_GEOMETRIC_OP = 265,      // "geometric_op"
    TOKEN_ARITHMETIC_OP = 266,     // "arithmetic_op"
    TOKEN_OPERATOR = 267           // "operator"
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_COMMA = 8,                             // "comma"
        S_ASSIGN = 9,                            // "assignment"
        S_GEOMETRIC_OP = 10,                     // "geometric_op"
        S_ARITHMETIC_OP = 11,                    // "arithmetic_op"
        S_OPERATOR = 12,                         // "operator"
        S_YYACCEPT = 13,                         // $accept
        S_program = 14,                          // program
        S_assignmentRule = 15,                   // assignmentRule
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::TOKEN_END || tok == token::TOKEN_YYerror || tok == token::TOKEN_YYUNDEF || tok == token::TOKEN_LEFTPAR || tok == token::TOKEN_RIGHTPAR || tok == token::TOKEN_SEMICOLON || tok == token::TOKEN_COMMA || tok == token::TOKEN_ASSIGN);
      }
#else
      ...
      symbol_type (int tok, std::string v, location_type l)
        : super_type(token_type (tok), std::move (v), std::move (l))
      {
        YY_ASSERT (tok == token::TOKEN_ID || tok == token::TOKEN_GEOMETRIC_OP || tok == token::TOKEN_ARITHMETIC_OP || tok == token::TOKEN_OPERATOR);
      }
#else
