...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::TOKEN_END || tok == token::TOKEN_YYerror || tok == token::TOKEN_YYUNDEF || tok == token::TOKEN_LEFTPAR || tok == token::TOKEN_RIGHTPAR || tok == token::TOKEN_SEMICOLON || tok == token::TOKEN_COMMA || tok == token::TOKEN_PLUS);
      }
#else
